...
Config LoadConfig(const std::string& aConfigFilePath)
{
    ...
    std::getline(lInputFile, lTempString);
    lReturnConfig.HarmonicCount = std::stoi(lTempString);
...
void PrintConfig(const Config& aConfig)
{
    ...
    std::cout << "DOF count: " << aConfig.DOFCount << std::endl;
    std::cout << "Harmonic waves count: " << aConfig.HarmonicCount << std::endl;
    ...
    std::cout << "Total HBM dof count: " << aConfig.DOFCount * (2 * aConfig.HarmonicCount - 1) << std::endl;
    std::cout << lBorder << std::endl;
}
