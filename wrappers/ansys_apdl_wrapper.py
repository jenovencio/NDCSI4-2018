...
class pyAPDL():
    def __init__(self,filepath=None,temp_folder_prefix='temp_', result_folder = ''):
    ...
    def read_result(self,filepath):
        ...
        self.results_dict[filepath.split('\\')[-2]] = df
        return df
    ...
    def read_frequencies(self,filename_prefix='Frequency_harm_',ext='.txt'):
        ...
        for h_index in range(1,num_harmonics+1):
            filename = filename_prefix + str(h_index) + ext
            filepath = os.path.join(self.result_folder,filename)
            df = self.read_result(filepath )
            df_key= df.columns[0]
    ...
    def read_mode_shapes(self, harm_prefix='Harm_', mode_prefix = '_mode', ext='.txt'):
        ...
        self.modes_list = []             
        for h_index in range(1,num_harmonics+1):
            self.modes_dict[h_index] = {}
            for mode_index in range(1,num_modes+1):
                filename = harm_prefix + str(h_index) + mode_prefix + str(mode_index) +  ext
                filepath = os.path.join(self.result_folder,filename)
                df = self.read_result(filepath)
                ux = list(df['UX'])
                uy = list(df['UY'])
                ...
                all = np.array([ux,uy,uz]).T
                mode_shape = all.flatten()
                self.modes_dict[h_index][mode_index] = mode_shape
                self.modes_list.append(mode_shape)
